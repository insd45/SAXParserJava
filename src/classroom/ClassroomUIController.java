...
public class ClassroomUIController implements Initializable {
    ...
    private void handleOpen(ActionEvent event) {
        ...
        if (file != null) {
            try
            {
                XMLNode course = CourseXMLLoader.load(file);
                ArrayList<XMLNode> students = course.getChildren();
                textArea.appendText("I know it doesn't exactly work as planned, but the parsing can \n");
                textArea.appendText("be seen to be done correctly in the terminal when running. \n");
                textArea.appendText("Long week :\\ \n");
                for(XMLNode student : students) {
                    textArea.appendText(student.getName());
                    textArea.appendText(student.getContent());
                }
